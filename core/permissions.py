...
LOG = logging.getLogger('eoapi.' + __name__)


class ExDjangoModelPermissions(rf_permissions.DjangoModelPermissions):
    """
...
class IsOwnerPermissions(rf_permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        full_permission_roles = getattr(view, 'full_permission_roles', False)

        #: Allowing permission for event manager roles
        if full_permission_roles and request.user.role in full_permission_roles:
            return True
...
class CsrfExemptSessionAuthentication(SessionAuthentication):
    """
    Overiding CSRF check in Session authentication
    """
